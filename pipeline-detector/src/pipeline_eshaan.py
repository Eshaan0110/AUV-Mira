...
# =====================================================================
# NEW: Underwater preprocessing helpers
# =====================================================================
...
def enhance_underwater(bgr, use_clahe=True, use_specular=True,
    ...
    if use_specular:
        ...
        # Dilate a bit so the inpaint reaches into the bright halo, not
        # just the saturated core
...
def detect_yellow_pipeline(frame, hsv_lower, hsv_upper,
    ...
    # Clean up noise. Close kernel is bigger now (was 11) and elliptical
...
def main():
    ...
    hsv_lower = np.array([25, 150, 215])  # was [15, 100, 100]
    hsv_upper = np.array([50, 255, 255])  # was [35, 255, 255]
    ...
    cv2.namedWindow('Preprocessed', cv2.WINDOW_NORMAL)   # NEW
    ...
    # NEW: preprocessing toggles + tuning
    ...
    while True:
        ...
        if ret and frame is not None:
            ...
            # NEW: read preprocessing toggles
            ...
            # Build the preprocessed view once so we can display it AND
            # feed it to the detector without doing the work twice.
            proc = enhance_underwater(frame,
                                      use_clahe=use_clahe,
                                      use_specular=use_specular,
                                      use_bilateral=use_bilateral,
            ...
            # DETECT PIPELINE (preprocess=False because we already did it)
            ...
            # DETECT ARUCO MARKERS  (ArUco still runs on the raw frame -
            # CLAHE-on-grey inside that class already handles its own lighting)
            ...
            cv2.imshow('Preprocessed', proc)        # NEW - see what HSV sees
